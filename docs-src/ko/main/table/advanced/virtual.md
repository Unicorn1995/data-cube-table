...
::: tip 자동 재계산 시기
아래 데이터 변경 시 가시 영역이 자동으로 재계산되므로 수동 호출이 필요하지 않습니다:

* `dataSource` 변경(새 배열로 교체)
* 로컬 필터링(`setFilter`, `remote: false`)
* 정렬(헤더 클릭 정렬, `setSorter`, `resetSorter`)
* 트리 펼치기/접기, 행 펼치기(`setTreeExpand`, `setRowExpand`)
:::
