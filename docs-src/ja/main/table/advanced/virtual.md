...
::: tip 自動再計算のタイミング
以下のデータ変更では可視領域が自動的に再計算されるため、手動で呼び出す必要はありません。

* `dataSource` の変更（配列の差し替え）
* ローカルフィルタリング（`setFilter`、`remote: false`）
* ソート（ヘッダークリックによるソート、`setSorter`、`resetSorter`）
* ツリーの展開/折りたたみ、行の展開（`setTreeExpand`、`setRowExpand`）
:::
