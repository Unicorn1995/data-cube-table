...
var StkTable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const [virtualScroll, virtualScrollX, virtual_on, virtual_dataSourcePart, virtual_offsetBottom, virtualX_on, virtualX_columnPart, virtualX_offsetRight, tableHeaderHeight, initVirtualScroll, initVirtualScrollY, initVirtualScrollX, updateVirtualScrollY, updateVirtualScrollX, setAutoHeight, clearAllAutoHeight, clearColWidthCache] = useVirtualScroll(props, tableContainerRef, trRef, dataSourceCopy, tableHeaderLast, tableHeaders, rowKeyGen, maxRowSpan, scrollbarOptions, isExperimentalScrollY);
		...
		const cellStyleMap = computed(() => {
			...
			for (let depth = 0, depthLen = headers.length; depth < depthLen; depth++) {
				...
				for (let i = 0, colsLen = cols.length; i < colsLen; i++) {
					const col = cols[i];
					const styleStr = `--cw:${virtualX ? getCalculatedColWidth(col) + "px" : transformWidthToStr(col.width)};min-width:${transformWidthToStr(col.minWidth)};max-width:${transformWidthToStr(col.maxWidth)}`;
					const colKey = colKeyGenValue(col);
					...
					createElementVNode("tbody", {
						...
						(openBlock(true), createElementBlock(Fragment, null, renderList(unref(virtual_dataSourcePart), (row, rowIndex) => {
							return openBlock(), createElementBlock("tr", mergeProps({
								...
								}, [createElementVNode("div", _hoisted_15, [renderSlot(_ctx.$slots, "expand", {
									row: row.__EXP_R__,
									col: row.__EXP_C__
								}, () => [createTextVNode(toDisplayString(row.__EXP_R__ && row.__EXP_C__ && row.__EXP_R__[row.__EXP_C__.dataIndex] || ""), 1)])])], 8, _hoisted_14)) : (openBlock(true), createElementBlock(Fragment, { key: 2 }, renderList(unref(virtualX_columnPart), (col, colIndex) => {
									return openBlock(), createElementBlock(Fragment, null, [!shouldHideCell(row, col) ? (openBlock(), createElementBlock("td", mergeProps({ key: colKeyGen.value(col) }, { ref_for: true }, getTDProps(row, col, rowIndex, colIndex)), [col.customCell ? (openBlock(), createBlock(resolveDynamicComponent(col.customCell), {
										key: 0,
										class: "table-cell-wrapper",
										tabindex: "-1",
										col,
