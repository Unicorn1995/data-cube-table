...
function insertToOrderedArray(sortState, newItem, targetArray, sortConfig = {}) {
	...
	if (emptyToBottom && isEmptyValue(targetVal)) data.push(newItem);
	else {
...
function getClosestColKey(target) {
	var _target$closest;
	return target === null || target === void 0 || (_target$closest = target.closest("td")) === null || _target$closest === void 0 ? void 0 : _target$closest.dataset.colKey;
}
/**
	...
	*/
	const getFixedColWidths = computed(() => {
		...
		for (let i = 0, j = cols.length - 1; i < cols.length; i++, j--) {
			...
			if ((leftCol === null || leftCol === void 0 ? void 0 : leftCol.fixed) === "left") {
				leftSum += getCalculatedColWidth(leftCol);
				leftAccumulated.push({
					i,
					w: leftSum
				});
			}
			if ((rightCol === null || rightCol === void 0 ? void 0 : rightCol.fixed) === "right") {
				rightSum += getCalculatedColWidth(rightCol);
				rightAccumulated.unshift({
					i: j,
					w: rightSum
				});
			}
		...
		return (colIndex) => {
			...
			for (let i = leftAccumulated.length - 1; i >= 0; i--) if (leftAccumulated[i].i < colIndex) {
				leftFixedWidth = leftAccumulated[i].w;
				break;
			}
			let rightFixedWidth = 0;
			for (let i = rightAccumulated.length - 1; i >= 0; i--) if (rightAccumulated[i].i > colIndex) {
	...
	function onSelectionMouseDown(e) {
		...
		if (e.shiftKey && anchorCell) selectionRange.value = {
	...
	function updateSelectionFromPoint(container, containerRect) {
		...
		if (!el) return;
		const td = (_el$closest = el.closest) === null || _el$closest === void 0 ? void 0 : _el$closest.call(el, "td");
		const tr = (_el$closest2 = el.closest) === null || _el$closest2 === void 0 ? void 0 : _el$closest2.call(el, "tr");
		if (!td || !tr) return;
	...
	function emitSelectionChange() {
		const range = selectionRange.value;
		if (!range) {
			emits("area-selection-change", null, {
				rows: [],
				cols: []
			});
			return;
		...
		const data = dataSourceCopy.value;
		const cols = tableHeaderLast.value;
		emits("area-selection-change", range, {
...
var ON_DEMAND_FEATURE = { [useAreaSelection.name]: (() => {
...
var TagType = /* @__PURE__ */ function(TagType) {
	TagType[TagType["TH"] = 0] = "TH";
	TagType[TagType["TD"] = 1] = "TD";
	/** tfoot */
	TagType[TagType["TF"] = 2] = "TF";
	return TagType;
}({});
//#endregion
...
function useFixedStyle(props, isRelativeMode, getFixedColPosition, virtualScroll, virtualScrollX, virtualX_on, virtualX_offsetRight) {
	...
	*/
	function getFixedStyle(tagType, col, depth = 0) {
		...
		else if (tagType === TagType.TF) style.bottom = "0px";
...
function useHighlight(props, stkTableId, tableContainerRef) {
		...
		const recursion = () => {
			window.requestAnimationFrame(() => {
				...
				highlightDimRowsAnimation.forEach((store, rowKeyValue) => {
					...
					if (timeOffset < duration) updateRowAnimation(rowKeyValue, store, timeOffset);
					else highlightDimRowsAnimation.delete(rowKeyValue);
				});
	...
	*/
	function setHighlightDimCell(rowKeyValue, colKeyValue, option = {}) {
		...
		else highlightCellsInCssKeyFrame(cellEl, rowKeyValue, className, duration);
	}
	/**
	* 高亮一行
	...
	*/
	function highlightCellsInCssKeyFrame(cellEl, rowKeyValue, className, duration) {
		...
		highlightDimCellsTimeout.set(rowKeyValue, window.setTimeout(() => {
			cellEl.classList.remove(className);
...
/** 翻页按键 */
var ScrollCodes = /* @__PURE__ */ function(ScrollCodes) {
	ScrollCodes["ArrowUp"] = "ArrowUp";
	ScrollCodes["ArrowRight"] = "ArrowRight";
	ScrollCodes["ArrowDown"] = "ArrowDown";
	ScrollCodes["ArrowLeft"] = "ArrowLeft";
	ScrollCodes["PageUp"] = "PageUp";
	ScrollCodes["PageDown"] = "PageDown";
	ScrollCodes["Home"] = "Home";
	ScrollCodes["End"] = "End";
	return ScrollCodes;
}(ScrollCodes || {});
/** 所有翻页按键数组 */
...
function useTableColumns(virtualX, isRelativeMode) {
	...
	*/
	function dealColumns(columns) {
		...
		if (maxDeep > 0 && virtualX) console.error("StkTableVue:多级表头不支持横向虚拟滚动!");
...
function useVirtualScroll(props, tableContainerRef, trRef, dataSourceCopy, tableHeaderLast, tableHeaders, rowKeyGen, maxRowSpan, scrollbarOptions, isExperimentalScrollY) {
	...
	*/
	function initVirtualScrollY(height) {
		...
		let scrollTop = ((_tableContainerRef$va = tableContainerRef.value) === null || _tableContainerRef$va === void 0 ? void 0 : _tableContainerRef$va.scrollTop) || 0;
		const rowHeight = getRowHeightFn.value();
...
var _hoisted_13 = ["onDrop"];
var _hoisted_14 = {
	key: 0,
	class: "vt-x-left"
};
var _hoisted_15 = ["colspan"];
var _hoisted_16 = { class: "table-cell-wrapper" };
...
var StkTable_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		*/
		...
		const [virtualScroll, virtualScrollX, virtual_on, virtual_dataSourcePart, virtual_offsetBottom, virtualX_on, virtualX_columnPart, virtualX_offsetRight, tableHeaderHeight, initVirtualScroll, initVirtualScrollY, initVirtualScrollX, updateVirtualScrollY, updateVirtualScrollX, setAutoHeight, clearAllAutoHeight] = useVirtualScroll(props, tableContainerRef, trRef, dataSourceCopy, tableHeaderLast, tableHeaders, rowKeyGen, maxRowSpan, scrollbarOptions, isExperimentalScrollY);
		...
		const [colResizeOn, isColResizing, onThResizeMouseDown] = useColResize(props, emits, tableContainerRef, tableHeaderLast, colResizeIndicatorRef, colKeyGen, fixedCols);
		...
		const { isSelecting: isAreaSelecting, onMD: onSelectionMouseDown, getClass: getAreaSelectionClasses, get: getSelectedArea, clear: clearSelectedArea, copy: copySelectedArea } = ON_DEMAND_FEATURE[useAreaSelection.name](props, emits, tableContainerRef, dataSourceCopy, tableHeaderLast, colKeyGen, cellKeyGen, scrollTo, virtualScroll, virtualScrollX);
		watch(() => props.columns, () => {
			handleDealColumns();
			updateMaxRowSpan();
			nextTick(() => {
		...
		function initDataSource(v = props.dataSource, option) {
			...
			if ((!props.sortRemote || (option === null || option === void 0 ? void 0 : option.forceSort)) && sortStates.value.length) {
				const sortConfig = {
					...DEFAULT_SORT_CONFIG,
					...props.sortConfig
				};
		...
		function filterDataSource(dataSource) {
			...
			return dataSource.filter((row) => {
				return filterKeys.every((key) => {
					...
					if (!(value === null || value === void 0 ? void 0 : value.length)) return true;
		...
		*/
		...
		function getTRProps(row, index) {
			...
			const rowKey = rowKeyGen(row);
			let classStr = props.rowClassName(row, rowIndex) || " " + ((row === null || row === void 0 ? void 0 : row.__EXP__) ? "expanded" : "") + " " + ((row === null || row === void 0 ? void 0 : row.__EXP_R__) ? "expanded-row" : "");
			if (currentRowKey.value === rowKey || row === currentRow.value) classStr += " active";
			if (props.showTrHoverClass && (rowKey === currentHoverRowKey.value || row === currentHoverRow)) classStr += " hover";
			const result = {
		...
		*/
		...
		function updateSortState(col, colKey) {
			...
			if (existingIndex >= 0) {
				...
				newOrder = sortSwitchOrder[(sortSwitchOrder.indexOf(currentOrder) + 1) % 3];
				if (newOrder === null) sortStates.value.splice(existingIndex, 1);
				else addOrUpdateSortState({
		...
		*/
		...
		function onCellMouseDown(e) {
			...
			if (props.areaSelection) onSelectionMouseDown(e);
		}
		...
		*/
		function onTableWheel(e) {
			...
			if (virtual_on.value && deltaY && !shiftKey) {
				...
				if (isWheeling()) e.preventDefault();
				if (isExperimentalScrollY.value) {
		...
		*/
		function onTableScroll(e) {
			if (!(e === null || e === void 0 ? void 0 : e.target)) return;
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				...
				createElementVNode("div", _hoisted_2, [createElementVNode("table", {
					...
					!__props.headless ? (openBlock(), createElementBlock("thead", _hoisted_3, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(tableHeaders), (row, rowIndex) => {
						...
						default: withCtx(() => [(openBlock(true), createElementBlock(Fragment, null, renderList(__props.footerData, (footRow, footRowIndex) => {
							return openBlock(), createElementBlock("tr", { key: footRowIndex }, [
								...
								(openBlock(true), createElementBlock(Fragment, null, renderList(unref(virtualX_columnPart), (col, colIndex) => {
									return openBlock(), createElementBlock("td", mergeProps({ key: colKeyGen.value(col) }, { ref_for: true }, getTFProps(col)), [col.customFooterCell ? (openBlock(), createBlock(resolveDynamicComponent(col.customFooterCell), {
										key: 0,
										class: "table-cell-wrapper",
										col,
										row: footRow,
									...
									}, [footRow[col.dataIndex] != null ? (openBlock(), createElementBlock("span", _hoisted_10, toDisplayString(footRow[col.dataIndex]), 1)) : createCommentVNode("", true)], 8, _hoisted_9)], 16);
								}), 128)),
					...
					createElementVNode("tbody", {
						...
						}, [unref(virtualX_on) && __props.fixedMode && __props.headless ? (openBlock(), createElementBlock("td", _hoisted_12)) : createCommentVNode("", true), __props.fixedMode && __props.headless ? (openBlock(true), createElementBlock(Fragment, { key: 1 }, renderList(unref(virtualX_columnPart), (col) => {
							return openBlock(), createElementBlock("td", {
								key: colKeyGen.value(col),
								style: normalizeStyle(cellStyleMap.value[unref(TagType).TD].get(colKeyGen.value(col)))
							}, null, 4);
						}), 128)) : createCommentVNode("", true)], 4)) : createCommentVNode("", true),
						(openBlock(true), createElementBlock(Fragment, null, renderList(unref(virtual_dataSourcePart), (row, rowIndex) => {
							return openBlock(), createElementBlock("tr", mergeProps({
								...
								row && row.__EXP_R__ ? (openBlock(), createElementBlock("td", {
								...
								}, [createElementVNode("div", _hoisted_16, [renderSlot(_ctx.$slots, "expand", {
									row: row.__EXP_R__,
									col: row.__EXP_C__
								}, () => {
									var _row$__EXP_R__;
									return [createTextVNode(toDisplayString(((_row$__EXP_R__ = row.__EXP_R__) === null || _row$__EXP_R__ === void 0 ? void 0 : _row$__EXP_R__[row.__EXP_C__.dataIndex]) ?? ""), 1)];
								})])], 8, _hoisted_15)) : (openBlock(true), createElementBlock(Fragment, { key: 2 }, renderList(unref(virtualX_columnPart), (col, colIndex) => {
									var _unref$rowKeyGen;
									return openBlock(), createElementBlock(Fragment, null, [!unref(hiddenCellMap) || !((_unref$rowKeyGen = unref(hiddenCellMap)[rowKeyGen(row)]) === null || _unref$rowKeyGen === void 0 ? void 0 : _unref$rowKeyGen.has(colKeyGen.value(col))) ? (openBlock(), createElementBlock("td", mergeProps({ key: colKeyGen.value(col) }, { ref_for: true }, getTDProps(row, col, rowIndex, colIndex), {
										...
										stkDragIcon: withCtx(() => [createVNode(DragHandle_default, { onDragstart: ($event) => unref(onTrDragStart)($event, getRowIndex(rowIndex)) }, null, 8, ["onDragstart"])]),
										_: 2
									}, 1032, [
										"col",
										"row",
									...
									}, toDisplayString((row && row[col.dataIndex]) ?? getEmptyCellText.value(col, row)), 9, _hoisted_17)) : col.type === "seq" ? (openBlock(), createElementBlock("div", _hoisted_18, toDisplayString((props.seqConfig.startIndex || 0) + getRowIndex(rowIndex) + 1), 1)) : col.type === "tree-node" ? (openBlock(), createBlock(TreeNodeCell_default, {
										key: 3,
										class: "table-cell-wrapper",
										col,
										row,
									...
									}, null, 8, ["onDragstart"])) : col.type === "expand" ? (openBlock(), createBlock(TriangleIcon_default, {
										key: 1,
										onClick: ($event) => triangleClick($event, row, col)
									}, null, 8, ["onClick"])) : createCommentVNode("", true), row[col.dataIndex] != null ? (openBlock(), createElementBlock("span", _hoisted_20, toDisplayString(row[col.dataIndex]), 1)) : createCommentVNode("", true)], 8, _hoisted_19))], 16)) : createCommentVNode("", true)], 64);
								}), 256)),
					...
					onTouchstart: _cache[8] || (_cache[8] = (...args) => unref(onVerticalScrollbarMouseDown) && unref(onVerticalScrollbarMouseDown)(...args))
				...
				scrollbarOptions.value.enabled && unref(showScrollbar).x ? (openBlock(), createElementBlock("div", {
					...
					onTouchstart: _cache[10] || (_cache[10] = (...args) => unref(onHorizontalScrollbarMouseDown) && unref(onHorizontalScrollbarMouseDown)(...args))
				}, null, 36)) : createCommentVNode("", true)
			], 46, _hoisted_1);
		};
	}
